...
import AppRou from "./components/app-route/AppRou";
// import ModalWindow from "./components/UI/ModalWindow";
// import InfoForStudents from "./components/main/info-students/InfoForStudents";
// import StudentBlock from "./components/main/student-block/StudentBlock";
// import TutorBlock from "./components/main/tutor-block/TutorBlock";
...
const App=()=> {
  ...
  const [toggleMain, setToggleMain] = useState(null);
  ...
  const [
    ...
    setDescendOrderPaymentStatusForStudCard,
  ] = useState("");
...
const toggleMainHandler = () => {
  ...
  //////////////////////////////////////////////
  const upliftDataForStudOrderCardHandler = (
    orderDataforStudOrderCardWithPayment
  ) => {
    console.log(orderDataforStudOrderCardWithPayment);

    const idForStudentOrderCard =
      orderDataforStudOrderCardWithPayment.studentOrderId;
    const tutDataForStudentOrderCard =
      orderDataforStudOrderCardWithPayment.tutorForStudentList;
    const orderSubjForStudentOrderCard =
      orderDataforStudOrderCardWithPayment.subject;
    const stateForStudentOrderCard =
      orderDataforStudOrderCardWithPayment.upToDateState;
    const startDateForStudentOrderCard =
      orderDataforStudOrderCardWithPayment.startDate;
...
console.log(convertData)
  ...
  const getTutorDataListHandler =
    ...
    async () => {
      ...
      try {
        const response = await fetch(
          `${reactAppHttp}${reactAppHost}${reactAppPort}/api${reactAppUrlApi}/users/tutors?size=70`,
        ...
        setErr({
          ...
          message:
            "Что-то пошло не  так!..Зарегистрированных тьютеров не найдено из-за возможной ошибки сервера!..",
        });
      }
      setIsLoading(false);
    };
  ...
  const getOrdersForStudentHandler = async () => {
    ...
    try {
      ...
      setErr({
        title: "Что-то пошло не так!",
        message:
          "Что-то пошло не  так!..Заказов не найдено:возможна ошибка сервера!..",
      });
    }
    setIsLoading(false);
  };
...
// const checkSignupHandler=(jwtTocken)=>{
//   if(jwtTocken){
//     setCheckLogin(null);
//     console.log("Форма ЛОгинации не выскакивает!")
// }  else if (!jwtTocken){
//   setCheckLogin(true);
//   console.log("Форма ЛОгинации всплывает!")
      ...
      {/* <Switch> */}
          <header className={styles.header}>
            <Header
              ...
              onToggleMain={toggleMainHandler}
            />
          </header>

          {!toggleMain && <AppRou
          // isAuthenticated={isAuthenticated}
              //  onCheckSignup={checkSignupHandler}
               onToggleMain={toggleMainHandler}
               users={userList}
              //  onSubjectForPC={onSubjectInMain}
               ...
               onFetchStudentCourseDetails={postStudentCourseDetailsHandler}
                />}
          {!toggleMain && <RedirectionModal onToggleMain={toggleMainHandler} />}
...
<Router>
        ...
        //  setIsAuthenticated={setIsAuthenticated}
            ...
            <Route path="/RedirectionModal" element={<RedirectionModal />}/>
            <Route path="/About" element={<About />}/>
            <Route path="/Conditions" element={<Conditions />}/>
            <Route path="/LawSubstrate" element={<LawSubstrate/>}/>
            <Route path="/ClientsFeedbacks" element={<ClientsFeedbacks />}/>

            <Route path="/TutorMarketWithFilter" element={<TutorMarketWithFilter
               convertData={convertData}
                onTutorList={tutorDataList}
            ...
            <Route path="/ParamsSelectedTutOrderCard" to>
              <ParamsSelectedTutOrderCard
                ...
                stateSelectedOrderForTutorCard={
                  stateSelectedOrderForTutorCard
                }
                startDateSelectedOrderForTutorCard={
                  startDateSelectedOrderForTutorCard
...
export default App;
          ...
          <div className={styles.rightMainContent}> */}
            ...
            {/* {isLoading && <p>Происходит загрузка данных пользователей uiuiuiui...</p>} */}
