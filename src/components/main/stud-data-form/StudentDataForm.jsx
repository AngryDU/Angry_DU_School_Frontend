  ...
  const submitStudentDataHandler = (event) => {
    ...
    if (
      ...
      setError({
        ...
        message:
          "Эти поля не могут быть пустыми, иначе Ваш профиль не сможет найти отклик!",
      });
      return;
    }
    ...
    if ( studentContactDataObj.aboutYourself.trim().length > 450) {
     setError({
       title: "Некорректный ввод!",
       message:
         "Это поле не может превышать 450 знаков!",
     });
    setFormIsValid(localStorage.getItem("validation"));
     return;
   ...
   console.log(formIsValid, aboutYourselfIsValid, validated, addressIsValid);
    ...
    setInputFirstName("");
    setInputUserSubject("");
    onGatheringStudentData(studentContactDataObj);
    setInputUserLevel("");
    setInputUserPhone("");
